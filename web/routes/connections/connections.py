...
from daqbroker.web.routes.utils import get_current_user, Depends, HTTPException, oauth2_scheme
...
#@app.get("/{connection}", response_model=ConnectionData)
...
#async def add_user(new_user: ConnectionInput, current_user: User = Depends(get_current_user)):
#	current_user.test_security(level=3)
#	connection = get_local_by_attr(db= session, Resource= User, attr_name= "username", attr_val= new_user.username)
#	if user:
#		raise HTTPException(
...
#@app.put("/{username}", response_model=ConnectionData)
