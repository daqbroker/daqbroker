				...
				if(functionArguments[0]=='step2'){
					...
					$("#requestedInst").find('.step2').find('.routinePicker').change(function(){
						...
						$.ajax({
							...
							success: function(returned){
								$(".step2").find(".firstStep").empty();
								$("#requestedInst").find(".firstStep").append('<p class="welcome">Your instrument provides some sort of data output. Please describe each source of data (you can repeat this process multiple times)</p><p>Routine name : <input class="name selectMetaStart"></input></p><p> Node : <select class="node selectMetaStart"><option value="-9999" selected disabled>--- SELECT ONE ---</option></select></p> <p> Data type : <select class="type selectMetaStart"><option value="-9999" selected disabled hidden>--- SELECT ONE ---</option><option value="0">Data file(s)</option><option value="1">Network stream (port communication)</option><option value="2">Serial (COM) port</option><option value="3" disabled>External program</option></select></p><div class="secondStep"></div>');
								var nodes=returned;
